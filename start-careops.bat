@echo off
REM CareOps Docker Startup Script for Windows
REM Usage: start-careops.bat [up|down|build|logs|restart]

set COMMAND=%1
if "%COMMAND%"=="" set COMMAND=up

echo ==========================================
echo CareOps - Docker Management Script
echo ==========================================

if "%COMMAND%"=="up" (
    echo Starting CareOps containers...
    docker-compose up -d
    echo.
    echo Waiting for services to start...
    timeout /t 10 /nobreak >nul
    echo.
    echo ==========================================
    echo CareOps is running!
    echo ==========================================
    echo Frontend: http://localhost:3000
    echo Backend:  http://localhost:8000
    echo API Docs: http://localhost:8000/docs
    echo.
    echo Login Credentials:
    echo   Owner: admin@careops.com / Admin@123
    echo   Staff: staff@careops.com / Staff@123
    echo ==========================================
    goto :end
)

if "%COMMAND%"=="down" (
    echo Stopping CareOps containers...
    docker-compose down
    goto :end
)

if "%COMMAND%"=="build" (
    echo Building CareOps containers...
    docker-compose build --no-cache
    goto :end
)

if "%COMMAND%"=="logs" (
    echo Showing CareOps logs...
    docker-compose logs -f
    goto :end
)

if "%COMMAND%"=="restart" (
    echo Restarting CareOps containers...
    docker-compose restart
    echo.
    echo CareOps restarted!
    echo Frontend: http://localhost:3000
    echo Backend:  http://localhost:8000
    goto :end
)

if "%COMMAND%"=="clean" (
    echo Cleaning CareOps containers and volumes...
    docker-compose down -v
    docker system prune -f
    echo Clean complete!
    goto :end
)

echo Usage: start-careops.bat [up^|down^|build^|logs^|restart^|clean]
echo.
echo Commands:
echo   up      - Start all services (default)
echo   down    - Stop all services
echo   build   - Rebuild all containers
echo   logs    - View logs
echo   restart - Restart services
echo   clean   - Remove containers and volumes

:end
