...
interface BasicDashboardLayoutProps {
  children: React.ReactNode
  pageTitle?: string
  breadcrumbs?: { name: string; href?: string }[]
}

export function BasicDashboardLayout({ children, pageTitle, breadcrumbs }: BasicDashboardLayoutProps) {
                ...
                <ol role="list" className="flex items-center space-x-2">
                  ...
                  {breadcrumbs.map((breadcrumb, index) => (
                    <li key={breadcrumb.name} className="flex items-center">
                      ...
                      {breadcrumb.href ? (
                        <Link href={breadcrumb.href} className="ml-2 text-sm font-medium text-neutral-500 hover:text-neutral-700">
                          {breadcrumb.name}
                        </Link>
                      ) : (
